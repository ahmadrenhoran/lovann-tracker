...
process.stdout.write(
  [
    ...
    `export N8N_INSTANCE_OWNER_EMAIL=${JSON.stringify(email)}`,
    `export N8N_INSTANCE_OWNER_FIRST_NAME=${JSON.stringify(firstName)}`,
    `export N8N_INSTANCE_OWNER_LAST_NAME=${JSON.stringify(lastName)}`,
    `export N8N_INSTANCE_OWNER_PASSWORD_HASH=${JSON.stringify(passwordHash)}`,
  ].join("\n"),
);
